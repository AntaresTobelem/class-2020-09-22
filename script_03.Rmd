...
- decide what data is actually relevant to the problem we want to solve (in this case, we need undergraduate enrollment data between Sept 1 and today)
- specifically define what we want to know (we want to understand enrollment trends)
- throwing out duplicates and unnecessary rows and columns to make our data easier to analyze


After we have a model, Temperance suggests we should:

- We must recognize the limitations of our model.
- We cannot ignore the uncertainty and unmodeled variation that come along with inference (We can show how enrollment data has changed, but we cannot explain why)
- We have to be aware of the fact that we cannot differentiate people who added vs. dropped the course.
- We must be careful given that students dropping the course may be fundamentally different from those adding the course, since courses are not randomly assigned.
